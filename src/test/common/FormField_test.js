...
describe('FormField', () => {
  ...
  describe('when only required props are passed', () => {
    it('renders a label with a div wrapper', () => {
      ...
      expect(component.html()).toEqual(
        '<div class=""><label class="">L1</label><div>Italy</div><div></div></div>'
      )
    })
  })
  ...
  describe('when errors are passed', () => {
    ...
    it('renders label and its wrapper with error classes', () => {
      ...
      expect(
        component
          .find('label.trouble-maker.input-error-label')
          .parent()
          .props().className
      ).toEqual('working-class object-oriented-class input-error')
    })
