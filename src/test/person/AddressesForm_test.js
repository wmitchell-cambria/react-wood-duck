...
import AddressesForm from '../../person/AddressesForm'
import './../EnzymeSetup'
...
describe('AddressForm', () => {
  ...
  it('renders a delete button for every address', () => {
    const addresses = [{}, {}]
    const component = renderAddressesForm({ addresses })
    const deleteLinks = component.find('button[aria-label="Delete address"]')
    expect(deleteLinks.length).toEqual(2)
  })

  it('passes default props to the delete button', () => {
    const addresses = [{}]
    const component = renderAddressesForm({ addresses })
    const deleteLink = component.find('button[aria-label="Delete address"]')
    expect(deleteLink.props().className).toEqual(
      'btn btn-default list-item__button'
    )
    expect(deleteLink.children('i').props().className).toEqual('fa fa-times')
  })

  it('calls delete address with the index of the address when the delete button is clicked', () => {
