...
describe('EthnicityForm', () => {
  const renderPersonEthnicityForm = ({
    ethnicityDetailOptions = [],
    ...options
  }) => {
    const props = { ethnicityDetailOptions, ...options };
    return shallow(<EthnicityForm {...props} />);
  };
  ...
  describe('Yes checkbox', () => {
    ...
    describe('ethnicityDetailOptions select field', () => {
      ...
      it('renders with no label if latinoOrigin is set to Yes', () => {
        ...
        expect(detailSelect.props().id).toEqual(
          'participant-123-ethnicity-detail'
        );
      });
  ...
  describe('Declined to answer checkbox', () => {
    it('renders a check box for Declined to answer', () => {
      const component = renderPersonEthnicityForm({ personId: '123' });
      const declinedBox = component.find(
        'CheckboxField[label="Declined to answer"]'
      );
      expect(declinedBox.exists()).toEqual(true);
      expect(declinedBox.props().id).toEqual(
        '123-ethnicity-declined-to-answer'
      );
    });
